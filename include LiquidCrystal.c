...
void loop() {
  ...
  if (state == 0) {
    ...
    if (currentMillis - previousMillis1 >= interval1) {
      ...
      for (int i = 0; i < diodCount; i++) {
        if ((i + modulo1) % (2 * diodCount) < diodCount) {
          digitalWrite(diody[i], HIGH);  // Zapnutí diody, pokud se nacházíme v rozsahu prvních numDiod pozic
        } else {
          digitalWrite(diody[i], LOW);  // Vypnutí diody, pokud se nacházíme mimo tento rozsah
        }
      }

      modulo1++;  // Inkrementace proměnné modulo pro posunutí diod
    }
    if (modulo >= maxState) {
      modulo = 0;
    }
    ...
    if (currentMillis - previousMillis1 >= interval1) {
      ...
      digitalWrite(diody[currentLED], 1);
      currentLED += direction;
      if (currentLED == diodCount) {
        direction = -1;
      }
      if (currentLED == 0) {
        direction = 1;
